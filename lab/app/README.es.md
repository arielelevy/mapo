...
| `guards.py` | **La política de guardas**: los 17 límites de todos los patrones, en un solo lugar y agrupados por **lo que gobiernan** —en cuántos pedazos se corta el problema, cuánto ve cada pedazo, cuántas vueltas tiene, cuándo se deja de insistir, cuánto estado se arrastra. | Ejecutado, y **no cambió ningún valor**: es una mudanza. Lo que habilita es lo que antes no se podía formular — que `handoff` corta en 2, `dag` en 4 y `supervisor` en 4-sobre-8 son **la misma decisión tomada tres veces**, y ninguna se midió como factor. `test_science.py` §66 impide que se vuelvan a desparramar. |
