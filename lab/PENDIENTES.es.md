...
| P-2 | El peso Hebbiano: usarlo o sacarlo | `policy.py` lo actualiza y `router.py` no lo lee **ni una vez**. Medido: seleccionar por peso, por utilidad media o por tasa de victorias elige idéntico. Es registro, no política — y una afirmación que no gobierna nada no debería seguir en el camino activo |
