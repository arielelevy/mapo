...
- [ ] **P27e** · **la ganancia plana mata el argumento de ruteo por dificultad**, que era la base de `X-5b`/`X-5f`. Si el caro gana igual en piso y en margen, elegirlo por región no compra nada: la decisión es de **presupuesto** (pagar siempre o nunca) y no de ruteo. Antes de dar eso por cerrado hay que ver si el eje equivocado era la dificultad — `X-5e` midió que la ventaja depende del **corpus** (0,81× en `gold_deep`, 1,90× en `gold_v2`), y eso es otra partición
