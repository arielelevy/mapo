...
- [ ] **P27h** · **implementar la política de modelo en `modelo × paradigma`.** Hoy `Router.plan` elige el modelo por **regla** —piso del dial, después presupuesto— y no aprende nada. `PolicyBundle` tiene `stats` por `(región, paradigma)`; hace falta un segundo mapa por `(modelo, paradigma)` **dentro del bundle firmado**, con la misma guarda de promoción. No es un campo más: es una segunda política, y tiene que poder promoverse y revertirse por separado
