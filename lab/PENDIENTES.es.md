...
- [ ] **X-5o** · **la VENTANA de contexto sigue puesta a mano** en `app/models.py` (400.000 para los dos). El precio ya salió del código y la ventana no: es una declaración del proveedor que no verifiqué, y la cota de `check_pair` la consume. Sale de la misma API que los precios, o del `list-models`
- [ ] **X-5m** · **`luna` al mismo precio que `nano` es SUSTITUCIÓN, no ruteo.** Si es más capaz al mismo costo, no hay decisión que tomar por request: se cambia el barato del catálogo y listo. Se decide con una corrida pareada chica —`rewoo` sobre las 8 tareas que discriminan, ~190k tokens— y es la más barata de todas las pendientes
