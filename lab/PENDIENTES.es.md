...
- [~] **U-2** · **REABIERTA: está implementada y nunca disparó** (lección 7.18). La precondición de cobertura poda por estructura, tiene test que pasa, y es **doblemente inerte**: `coverage_demanded` sólo lo declara `gold_p19` (20 de 407 tareas) y `TRAVERSES_SCOPE ∩ fila activa = ∅`, así que aunque disparara caería siempre en la rama «no se pudo imponer». **La cerré como hecha.** Para cerrarla de verdad hacen falta dos cosas: regenerar corpus con las demandas declaradas, y **decidir si algún brazo de la fila activa recorre el alcance** — hoy los únicos que recorren son `map_reduce` (despriorizado) y `direct` (sólo factible en ventana)
- [ ] **P-15** · **`shared_writes` no lo declara NINGUNA tarea de ningún corpus** (`_audit_inerte.py`), y sostiene el piso A2 entero: el nivel contable **nunca se alcanzó por esa vía en 407 tareas**. O se generan celdas que lo declaren, o el piso A2 por escrituras compartidas es una garantía que nadie midió
