...
- [ ] **X-5l** · **los precios reales de nano y de los `5.6` no los pude obtener** (pedido del autor): la API pública de precios de Azure devuelve **vacío** para `Cognitive Services` incluso sin filtro, y los `5.6` son nuevos. Los aranceles de `app/tariffs.py` siguen siendo **referencia declarada**, con la fecha adentro del nombre. Se cierran cuando el autor los saque del portal o de una factura
