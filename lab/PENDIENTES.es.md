...
- [~] **P-7** · **el producto ya deja rastro** (`store.append_decision` + base de creencias cuando el perfil lo declara, `test_science.py` §35). Y queda dicho el limite que no es obvio (leccion 8.8): produccion puede alimentar la **calibracion** —opinion contra observacion, las dos en el mismo request— pero **no la utilidad de θ**, porque un `Episode` lleva `was_best` y eso exige saber que habrian hecho los otros paradigmas. Produccion corre uno solo, y fabricar ese campo enseñaria que el brazo elegido siempre gana. Falta cablear el store en el endpoint real, que hoy no lo construye nadie
...
- [ ] **X-5** · **el presupuesto está en TOKENS y no debería** (planteo del autor, 2026-08-28). Un modelo más caro cuesta más por token **y debería resolver con menos llamadas**, así que contar tokens mezcla dos cosas que se mueven en sentidos opuestos. Hoy `cost_ratio` normaliza contra el brazo más barato **del mismo modelo**: cruzando modelos, numerador y denominador se mueven juntos y el número deja de significar nada. Y el spread ya existe *dentro* de un modelo — `rewoo` promedia **2.435** tokens por celda contra **89.800** de `dag_strategy`, un factor **37×**. Un modelo 10× más caro por token que necesitara 37× menos tokens **saldría más barato en plata**, y el banco hoy no puede ni preguntarlo
- [ ] **X-5a** · **la unidad es PLATA, y está medio construida.** El split prompt/completion ya vive en la fila (`X-2`) y la tarifa se declara por entorno; falta que λ barra sobre `prompt·precio_in + completion·precio_out` en vez de sobre el total. **Y el piso del ratio tiene que ser global**, no por modelo: si el brazo más barato de A cuesta $0,01 y el de B $0,10, ratios por modelo hacen que los dos parezcan igual de eficientes. Es la misma clase de error de normalización que este banco ya agarró tres veces
- [ ] **X-5b** · **el modelo es ACCIÓN, no estado — y esto no es un detalle.** El espacio de decisión pasa a ser el par `(modelo, paradigma)`, y la factibilidad poda **pares**. Meterlo en la **región** sería el error opuesto y ya está medido lo que cuesta: agregar un cuarto segmento al vocabulario le costó a θ toda su confianza —cada episodio cayó en un bin demasiado chico para cruzar el piso de evidencia— y ése fue el mecanismo de `P15`. El modelo se **elige**, no se **observa**
- [ ] **X-5c** · **la calidad no es presupuesto: es lo que se compra.** Pero hay una restricción real y ya existe el lugar donde vive: **el dial**. Hoy A2/A3 restringen los *paradigmas* admisibles; tienen que restringir también los **modelos** — una acción irreversible no puede rutearse al modelo más barato porque salga la cuenta. Eso resuelve «token **y** calidad» sin meter la calidad adentro del costo: **plata** es el presupuesto, **piso de capacidad** es una precondición
- [ ] **X-5d** · **la comparación entre modelos va en el ANÁLISIS, nunca en las filas.** `load_rows` levanta si un archivo mezcla decodificaciones, y **está bien**: promediar entre modelos no mide un paradigma, mide el modelo. El estudio multi-modelo se arma **uniendo estudios por modelo**, no juntando filas. Queda escrito para que nadie «arregle» la guarda creyendo que estorba
- [~] **X-5e** · **contestada con lo ya pagado** (`_analyze_two_models.py`, lección 5.4): la hipótesis se confirma **condicionada a la dificultad**. En `gold_deep` el modelo caro usa **0,81×** los tokens del barato —resuelve con menos— y en `gold_v2` usa **1,90×**. Puntos de equilibrio **1,24×** y **0,53×** por token. Sobrio: aun donde gana, 1,24× no alcanza para pagarse — y eso **fortalece** el caso del ruteo, porque el uso correcto del caro es exactamente donde gana (3 de 6 y 3 de 8 tareas, con al menos una en cada corpus donde **el barato alcanza más alto**). Falta la **tarifa** para convertirlo en plata
- [ ] **X-5f** · **dos modelos en el sistema** (propuesta del autor, 2026-08-28): uno rápido y uno caro, eligiendo en cada caso. Es `X-5b` hecho producto. Lo que la medición agrega es **por qué eje se elige**: la ventaja del caro depende de la **dificultad**, que es lo que la región ya mide (cardinalidad · acoplamiento · continuación). Y una trampa a evitar: el modelo va al **espacio de acción**, no al vocabulario de región — meterlo ahí multiplica los bins y le cuesta a θ toda su confianza, que fue el mecanismo de `P15`
...
- [~] **D-1** · **P20 CORRIDA, y mayormente refutada.** `P20a` **REFUTADA**: el costo baja **2,6%**, no el ≥10% predicho. `P20d` **REFUTADA** y explica a la primera: tras el rechazo el modelo **re-emite la búsqueda el 69% de las veces**. `P20b` **CONFIRMADA** —cortar no cuesta utilidad (caída 0,0057 contra piso 0,0773)—, y `P20c` confirmada pero sin significar mucho: `react`, el bucle más puro, tuvo **cero rechazos**. Sigue abierto porque la corrección es estructural y está identificada (`D-1c`)
...
- [~] **D-5b** · **derivadas** (lección 5.6). Los `COST_PRIORS` **erran 2-7×** — `reflection` dice 5× y mide **1,6×**. Y lo primero fue averiguar **quién consume el número**, porque la respuesta obvia era la equivocada: **NO gobiernan la poda** —`feasibility.admissible()` no los mira— sino **el orden de la cascada**. Con `reflection` inflado, la cascada **nunca lo prueba primero** aunque sea de los más baratos, y la cascada es a la que el paper le acredita el 100% de la brecha. Y `max_iterations` de `react` es **20** cuando el p95 de las réplicas que aciertan es **4**: no acota nada
- [ ] **D-5c** · **la escala está anclada a un brazo que no corre en el régimen que el producto apunta.** `direct` es el denominador y **sólo es factible donde todo entra en ventana** — 7 tareas en 1.008 filas. La referencia tiene que ser **el brazo factible más barato de esa tarea**, que es lo que `metrics.cost_ratio` ya usa para puntuar: ahí la escala existe en todo régimen. Y el prior es transitorio por diseño —el costo medido lo supera— así que el error vive donde θ no aprendió: **cada despliegue y cada región nueva**
