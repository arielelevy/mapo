...
- [x] **P-14b** · **la guarda de mezcla no protegía a nadie que analizara** (lección 5.8): vivía adentro de `Runner`, y **todo analizador lee el `.jsonl` a mano**. Es la forma del barrido 7.17 cometida **en la corrección misma** — la guarda de vocabulario se escribió hoy y se puso donde no tapa nada. `load_rows` es función de módulo; `Runner` delega. Y apenas aplicada agarró que `results/nano/beliefs/` **no es un archivo de resultados**: 207 líneas que se excluyen declarándolas
