...
- [~] **L-4** · **rellenado desde caché** (`bench/runs/_backfill.py`, lección 8.10). Tres defectos en veinte líneas cuyo único trabajo es **no gastar**: (1) la guarda miraba `cost_tokens`, que un acierto de caché **reporta igual que la llamada original** — habría reventado sobre un rellenado perfecto; (2) `cached_calls` existía en `Usage` y **no llegaba a `Row`**, así que no se podía distinguir pagado de replayado; (3) tomaba el alcance por argumento y la corrida original cubría 8 de 32 tareas — **empezó a pagar las otras 24**, y la copia previa salvó el registro. Ahora el alcance **se lee del archivo** y la guarda mide `calls - cached_calls`
