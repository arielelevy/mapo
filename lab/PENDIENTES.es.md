...
- [ ] **X-5j** · **habilitar el modelo caro en Foundry** (planteo del autor). El código está listo: `app/models.py` tiene el catálogo, `Capability` el piso ordinal y `Router.plan(models=…)` la selección por request. Falta **un deployment real** y su entrada de configuración. Sin eso, `X-5f` está implementado y no se puede correr
