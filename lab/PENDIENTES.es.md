...
- [ ] **X-5i** · **auto-escalar al caro NO paga por falla** (lección 5.18): **72% de las celdas tienen `u<1`**, así que «escalar al fallar» ≈ «escalar siempre» — **19×** contra 26×. Y λ de indiferencia **0,030**, que además es cota optimista. Lo sano es **ex ante** por dificultad (`X-5b`/`X-5f`, ya construido). Lo que queda por medir: **cuánta falla atrapa un contrato que se niega** — es `COMPUTED`, no es la opinión del modelo, y es **rara por construcción**, que es la escasez que la economía exige
...
- [ ] **L-2** · **tiempo al PRIMER TOKEN, y no se puede medir hoy** (lección 5.20; planteo del autor). `llm.py` hace un POST y espera la respuesta entera: sin `stream=true` no hay evento que cronometrar. Es el número que el usuario percibe con la SSE que propone `ARQUITECTURA.es.md`. Cambia el cliente que produce **todo** el registro, así que no se improvisa. Y una trampa a evitar antes de empezar: un acierto de caché tiene TTFT **cero**, que no es el TTFT de nada — hay que separarlos o correr sin caché, igual que `wall_seconds` ya hace
- [ ] **L-3** · **la latencia entra a la decisión, o no sirve medirla.** Hoy `λ` barre sobre costo y el tiempo no entra en ninguna utilidad ni en ninguna cota. Un brazo que gana 0,030 y tarda 11,5× **no gana** para quien lo espera. Falta decidir si es un tercer eje de `λ`, una cota dura por request (como la ventana), o una precondición del dial — y las tres son decisiones distintas con consecuencias distintas
...
- [ ] **X-5p** · **BLOQUEANTE del ruteo de dos modelos: los `5.6` necesitan la Responses API para herramientas.** `ModelPool` ahora **se niega a construirse** con un modelo que no soporta tools sobre Chat Completions, y levanta al construir en vez de fallar a mitad de una corrida paga. Las tres salidas son decisiones distintas: (a) portar `llm.py` a la Responses API —toca el cliente que produce **todo** el registro—, (b) `reasoning_effort='none'` —apaga justo lo que se paga—, (c) elegir un modelo caro **no** de razonamiento. Hasta que se decida, `X-5f` está implementado y no se puede correr
- [ ] **X-5q** · **los tokens de razonamiento se facturan como SALIDA** y no aparecen en el contenido (`completion_tokens_details.reasoning_tokens`). Comparar precio por token entre un modelo que razona y uno que no es comparar **dos unidades distintas**: luna cobra 1,20 contra 1,25 de nano y emite tokens que nano no emite. El registro **no los separa** — `completion_tokens` los mete todos juntos
- [ ] **X-5n** · **tres ejes de precio que el modelo de costo no tiene**: contexto largo (**2× entrada, 1,5× salida**), entrada cacheada (**10× menos**) y **escritura de caché cobrada en los `5.6` y no en nano** — en `sol` la escritura sale más que la **entrada** de `terra`. El primero le da un gemelo económico a la cota de ventana; el tercero significa que un prefijo inestable paga sin cobrar. Documentados en `app/tariffs.py`, **no declarados como constantes** hasta que algo los consuma
...
- [ ] **D-3b** · **el acoplamiento no llega al paradigma**, y es la creencia que más gobernaría la forma del DAG: con acoplamiento alto, descomponer en paralelo mide cualquier cosa menos la cadena. La sonda **lo mide** y ni el `ToolSurface` ni la tarea la transportan, así que `dag_shape` registra `governed_by_coupling: False`. Es la forma del barrido 7.17 — una creencia completa que no llega a donde se decide
