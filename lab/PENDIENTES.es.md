...
- [ ] **P27g** · **si el efecto es del paradigma, θ tiene que aprender pares.** Hoy θ aprende por región sobre paradigmas y el modelo lo elige una regla. Con la ventaja concentrada en un brazo, la política correcta es `(modelo, paradigma)` por región — y eso **sí** multiplica bins, que es el mecanismo de `P15`. Antes de hacerlo hay que medir si el piso de evidencia aguanta la partición, no suponerlo
