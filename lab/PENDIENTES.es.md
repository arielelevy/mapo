...
- [ ] **P27i** · **la consolidación todavía no ALIMENTA `model_stats`.** La política existe, se firma, se lee y el router la usa — y nadie la escribe: hoy sólo se puebla a mano. Falta el brazo de `consolidation.py` que acumule episodios por `(paradigma, modelo)` con la misma guarda de promoción y el mismo registro de qué episodios ya entraron, o la segunda política es un mecanismo completo que ningún camino alimenta — exactamente lo que el barrido de la 7.17 busca
