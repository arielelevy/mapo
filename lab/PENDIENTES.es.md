...
- [x] **F-2** · **el estado compartido es un factor de la superficie** (`shared_state`), y `dag_strategy` lo honra sin cambiar su topología —mismas olas, mismo verify, mismos replans—. Pero **el cruce que pedía el pendiente no se puede construir**: en `react` es **degenerado** (un agente en un bucle; su transcripción ya es el estado, así que un «react con board» mediría redundancia) y en `map_reduce` **sería otro patrón** (vuelve secuenciales unas llamadas independientes, o sea cambia el grafo de control). Una celda vacía **por construcción** no es un hueco del banco. Escrito en `blackboard.py`
